...
void DFS(unsigned node,map<int,bool> &mvVisted, Graph const &gConnect){
  ...
  for( auto u = range.first; u != range.second; u++){  //iterate through the vertices initialized to fals because they havent been visted
    ...
    if( u->second == false){
      ...
      DFS(u->second,mvVisted,gConnect);    //we are seg faulting here
...
// unvisited vertices.
void DFSUtil(map<int,bool> mvVisted,Graph const &gConnect){

  for (auto i=mvVisted.begin(); i != mvVisted.end(); i++){
    if( i->second == false){
      DFS(i->second,mvVisted,gConnect);
    }
...
bool connected(Graph const &gConnect){
  ...
  DFS(*(gConnect.sVertices.begin()),mvistedVertex,gConnect);
