...
// Overloaded operator* to implement intersection of two graphs all elements in both lhs and rhs
...
Graph operator*(Graph const &lhs, Graph const &rhs){
  Graph intersectGraph;
   ...
   for(auto itl=lhs.sVertices.begin(); itl!=lhs.sVertices.end();itl++){       //itterate through the lhs
      ...
      if(canInsert) {intersectGraph.sVertices.insert(*itl);}
    }
    ...
    for (auto itl=lhs.mEdges.begin(); itl!=lhs.mEdges.end(); itl++){
      ...
      if(canInsert && intersectGraph.sVertices.find(itl->first)== intersectGraph.sVertices.end() && intersectGraph.sVertices.find(itl->second)== intersectGraph.sVertices.end()) {

        intersectGraph.mEdges.insert(uPair);
      }
    }
