...
for (auto itdg = differGraph.sVertices.begin(); itdg != differGraph.sVertices.end(); itdg++){
  bool canInsert = true;
  for(auto itl= lhs.mEdges.begin();itl!=lhs.mEdges.end();itl++){
    if(*itdg == itl->first){
      auto mdiffChk = lhs.mEdges.equal_range(itl->first);
      for (auto itdc=mdiffChk.first; itdc!=mdiffChk.second; ++itdc){
        for(auto itdg2 = differGraph.sVertices.begin(); itdg2 != differGraph.sVertices.end(); itdg2++){
          if(itdc->second == *itdg2){
            pair<unsigned,unsigned> ePair = make_pair(itl->first,*itdg2);
            differGraph.mEdges.insert(ePair);
            ePair = make_pair(*itdg2,itl->first);
            differGraph.mEdges.insert(ePair);
          }
        }
      }
