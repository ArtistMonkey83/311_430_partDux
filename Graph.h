...
//add(WeightedVertex source, unsigned destination, int edgeWeight=1): adds an edge between vertices source and destination with weight edgeWeight (which defaults to 1)
  bool add(WeightedVertex source, WeightedVertex destination, int edgeWeight=1){
    bool added = false;
    map<WeightedVertex,int> mlocal = make_pair(destination,edgeWeight); //local container to hold the goods
    //map<WeightedVertex,map<WeightedVertex,int>> mlocal2 = make_pair(source,mlocal);
    ...
    if(foundSource == this->sVertices.end())   //returns true if source isn't found, meaning we need to add the Vertex
    {
        this->sVertices.insert(source);     //insert the vertices that didn't exsist before
    }

    if(foundDestination == this->sVertices.end()){   //returns true if source isn't found, meaning we need to add the Vertex
      this->sVertices.insert(destination);
    }

    this->mEdges.emplace(source,mlocal);    //adding the edge only if statements returns false, meaning we didn't find this edge to already exist
    added = true;
...
//checking to see if an edge exsists between the source and destination vertex
    ...
    if(foundSource != this->sVertices.end()){   //returns true if source is found, meaning we might need to remove the Vertex
...
//remove edges some how
    ...
    if(foundDestination != this->sVertices.end()){   //returns true if source is found, meaning we need to remove the Vertex as well as the Edge
